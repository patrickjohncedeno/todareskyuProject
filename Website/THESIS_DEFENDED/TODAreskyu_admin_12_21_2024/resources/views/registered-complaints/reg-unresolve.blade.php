   ...
   <table class="table  m-0 table-hover rounded-3 align-middle">
    ...
    <thead class="border-dark">
        <tr class=" align-items-center " style="vertical-align:  middle;">
            ...
            <th scope="col" style="border-right: none;">Unresolved Reason</th>
            <th scope="col" style="border-left: none;"></th>
        </tr>
    </thead>
    <tbody c;>
        @foreach ($regUnresolved as $complaint)
            <tr class="position-relative" role="button" onclick="window.location='{{route('complaints.reg-unresolved-details', $complaint->complaint_registered_ID)}}'">
                ...
                <td class="text-truncate" style="max-width: 150px;">{{ $complaint->driver->driverName }}</td>
                <td class="text-truncate" style="max-width: 120px;">{{ $complaint->violations->violationName }}</td>
                <td>{{ \Carbon\Carbon::parse($complaint->dateResolve)->format('F j, Y') }}</td>
                <td class="text-truncate" style="max-width: 150px; border-right: none;">{{$complaint->resolutionDetail}}</td>
                <td class="ms-2" style="border-left: none; width: 30px">
                    ...
                    {{-- <span class=" position-absolute top-50 end-0 translate-middle-y action-icons">
                        <a href="{{ route('complaints.reg-unresolved-edit', $complaint->complaint_registered_ID) }}" style="text-decoration: none;">
