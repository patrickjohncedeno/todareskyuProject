    ...
    <thead class="border-dark">
        ...
        <tbody>
            @foreach ($complaints as $complaint)
                <tr class="position-relative" role="button" onclick="window.location='{{ route('complaints.reg-inprocess-show', $complaint->complaint_registered_ID)}}'">
                    <td class="text-truncate" style="max-width: 150px;">{{ str_pad($complaint->complaint_registered_ID, 3, '0', STR_PAD_LEFT) }}</td>
                    <td class="text-truncate" style="max-width: 150px;">{{ $complaint->user->firstName }} {{ $complaint->user->lastName }}</td>
                    <td class="text-truncate" style="max-width: 180px;">{{ $complaint->driver->driverName }}</td>
                    <td class="text-truncate" style="max-width: 150px;">{{ $complaint->violations->violationName }}</td>
                    <td>{{ \Carbon\Carbon::parse($complaint->meetingDate)->format('F j, Y') }}</td>
                <td class="ms-2" style="border-left: none; width: 30px">
                    ...
                    <span class=" position-absolute top-50 end-0 translate-middle-y action-icons">
                        <a href="{{ route('complaints.reg-inprocess-edit',$complaint->complaint_registered_ID) }}" style="text-decoration: none;">
