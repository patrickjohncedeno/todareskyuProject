...
@section('content')
    ...
    <div class="rounded-4 mb-5 overflow-hidden shadow">
        <table class="table m-0 table-hover rounded-3 align-middle">
            ...
            <tbody>
                @foreach ($unregResolved as $complaint)
                    <tr class="position-relative" role="button"
                        onclick="window.location='{{ route('complaints.unreg-settled-details', $complaint->complaint_unregistered_ID) }}'">
                        <td scope="row">{{ str_pad($complaint->complaint_unregistered_ID, 3, '0', STR_PAD_LEFT) }}</td>
                        <td class="text-truncate" style="max-width: 150px;">{{ $complaint->user->firstName }}
                            {{ $complaint->user->lastName }}</td>
                        <td class="text-truncate" style="max-width: 150px;">{{ $complaint->tricycleColor }},
                            {{ $complaint->tricycleDescription }}, {{ $complaint->plateNumber }}</td>
                        <td class="text-truncate" style="max-width: 200px;">{{ $complaint->violations->violationName }}</td>
                        <td class="text-truncate" style="max-width: 150px;">
                            {{ \Carbon\Carbon::parse($complaint->dateResolve)->format('F j, Y') }}</td>
                        <td>
                            @if ($complaint->paymentReceipt === null)
                                <span class="badge rounded-pill bg-warning text-dark">Pending</span>
                            @else
                                <span class="badge rounded-pill bg-success text-light">Receipt Uploaded</span>
                            @endif
                        </td>
                        <td>
                            <!-- Floating icons -->
                            <span class=" translate-middle-y action-icons">
                                {{-- <a href="{{ route('complaints.unreg-settled-edit', $complaint->complaint_unregistered_ID) }}"
