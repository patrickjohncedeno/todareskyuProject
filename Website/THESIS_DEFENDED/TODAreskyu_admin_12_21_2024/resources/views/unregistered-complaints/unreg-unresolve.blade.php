...
@section('content')
    ...
    <div class="rounded-4 mb-5 overflow-hidden shadow">
        <table class="table m-0 table-hover align-middle">
            ...
            <tbody>
                @foreach ($unregUnresolved as $complaint)
                    <tr class="position-relative" role="button" onclick="window.location='{{route('complaints.unreg-unresolved-details', $complaint->complaint_unregistered_ID)}}'">
                        ...
                        <td class="text-truncate" style="max-width: 150px;">{{$complaint->resolutionDetail}}</td>
                        <td>
                        <!-- Floating icons -->
                        <span class="translate-middle-y action-icons">
                            {{-- <a href="{{ route('complaints.unreg-unresolved-edit', $complaint->complaint_unregistered_ID) }}"
